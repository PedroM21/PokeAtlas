...
async function main() {
  try {
    ...
    const collection = db.collection("pokemons"); // collection name
    ...
    console.log(`${result.insertedCount} Pokemon inserted successfully!`);
  } catch (err) {
    console.error("Error importing Pokemon:", err);
  } finally {
    await client.close();
  }
}
